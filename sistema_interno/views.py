...
class EstoqueInnerView(
    ...
    def get(self, request):
        ...
        ctx = {
            ...
            "buffets": Cliente.objects.filter(ativo=True, tipo=Cliente.Tipo.BUFFET).order_by("nome_cliente"),
    ...
    def acao_save_cliente_rapido(self, request):
        """Mesmo cadastro completo usado dentro do orçamento."""
...
class MovimentacoesInnerView(RespostaJSONMixin, EstoqueInternoRequiredMixin, View):
    ...
    def acao_delete_movimento(self, request):
        if not request.user.is_superuser:
            return self.erro(request, "Somente o superusuário pode excluir movimentações.", status=403)
        movimento = get_object_or_404(
            MovimentoEstoque.objects.select_for_update().select_related("estoque"),
            pk=request.POST.get("movimento_id"),
        )
        exigir_confirmacao_exclusao(request)
        ultimo = movimento.estoque.movimentos.order_by("-ocorrido_em", "-id").first()
        if not ultimo or ultimo.pk != movimento.pk:
            raise ErroDeFormulario(
                "Por segurança, somente a movimentação mais recente deste item pode ser excluída. "
                "Exclua primeiro as movimentações posteriores."
            )
        estoque = movimento.estoque
        if movimento.tipo == MovimentoEstoque.Tipo.ENTRADA:
            estoque.quantidade = movimento.quantidade_resultante - movimento.quantidade
        elif movimento.tipo == MovimentoEstoque.Tipo.SAIDA:
            estoque.quantidade = movimento.quantidade_resultante + movimento.quantidade
        else:
            anterior = (
                estoque.movimentos.exclude(pk=movimento.pk)
                .order_by("-ocorrido_em", "-id").first()
            )
            if not anterior:
                raise ErroDeFormulario(
                    "Este ajuste não possui saldo anterior confiável para restauração. "
                    "Registre um novo ajuste corretivo em vez de excluí-lo."
                )
            estoque.quantidade = anterior.quantidade_resultante
        saldo = None
        if movimento.saldo_valor_resultante is not None and movimento.variacao_valor is not None:
            saldo = movimento.saldo_valor_resultante - movimento.variacao_valor
        if saldo is None:
            saldo = Decimal(str(estoque.preco_fornecedor or 0)) * estoque.quantidade
        estoque.saldo_valor = saldo
        estoque.preco_fornecedor = (
            (saldo / estoque.quantidade).quantize(Decimal("0.01"))
            if estoque.quantidade else Decimal("0.00")
        )
        estoque.custo_estimado = bool(estoque.quantidade and movimento.custo_estimado)
        estoque.save(update_fields=["quantidade", "saldo_valor", "preco_fornecedor", "custo_estimado", "atualizado"])
        identificacao = str(movimento)
        movimento.delete()
        return self.sucesso(request, f"Movimentação '{identificacao}' excluída e saldo restaurado.")
    ...
    def get(self, request):
        ...
        movimentos = (
            ...
            .annotate(
                ultimo_id_estoque=Subquery(
                    MovimentoEstoque.objects
                    .filter(estoque_id=OuterRef("estoque_id"))
...
class BuscaClientesEstoqueView(EstoqueInternoRequiredMixin, View):
    """Busca remota do estoque, com a mesma ficha usada no orçamento."""
    LIMITE = 20

    def get(self, request):
        termo = (request.GET.get("q") or "").strip()[:80]
        clientes = Cliente.objects.filter(ativo=True).select_related("parceiro").prefetch_related("enderecos")
        if termo:
            clientes = svc_clientes.buscar(clientes, termo).order_by("nome_cliente")
        else:
            clientes = clientes.order_by("nome_cliente")
        resposta = JsonResponse({
            "status": "sucesso",
            "opcoes": [svc_clientes.opcao_de_busca(c) for c in clientes[:self.LIMITE]],
        })
