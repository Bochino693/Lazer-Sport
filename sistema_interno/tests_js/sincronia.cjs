...
<button id="lsSom"></button><div id="avisosLista"></div><main class="ls-content">
...
(async()=>{
 ...
 const w=dom.window,d=w.document;let notas=0,emVoo=0,maxEmVoo=0,pedidos=0,versao='v2';
 ...
 w.fetch=async (url,opcoes={})=>{
   ...
   if(url.includes('/avisos/estado/')) {
     ...
     return resposta(estado(versao),url,{'ETag':'"'+versao+'"'});
   }
 ...
 w.Painel.confirmarGravacao();assert.equal(notas,2,'som após sucesso');
 d.querySelector('#lsSom').click();w.Painel.confirmarGravacao();assert.equal(notas,2,'silenciar respeitado');
