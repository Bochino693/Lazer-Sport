...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            for tabela, coluna, prefixo in (("auth_user", "id", "usuario"),
                ...
                for pk, email in cursor.fetchall():
                    if email and email.strip():
                        emails[email.strip().lower()].add(f'{prefixo} #{pk}')
        conflitos = [(email, donos) for email, donos in emails.items() if len(donos) > 1]
        for email, donos in conflitos:
            self.stdout.write(f'{email}: {", ".join(sorted(donos))}')
        ...
        if conflitos:
            raise CommandError(f'{len(conflitos)} e-mails em cadastros diferentes. Corrija os registros indicados antes de migrar; nada foi alterado.')
